...
claude-code hooks automation cli ai-development developer-tools

From the guide: [Claude Code Comprehensive Guide](/guides/claude-code)
...
-   **Solo developers:** Start with the auto-formatter (Hook 1) and security gate (Hook 2). These two hooks prevent the most common Claude Code mistakes with zero ongoing maintenance.
-   **Team leads:** Commit hooks to `.claude/settings.json` in your repo. Every team member gets the same safety gates and quality checks automatically.
-   **Security engineers:** Exit code 2 blocks the action.[2](#fn:2) Exit code 1 only logs a warning. Every PreToolUse security hook must use `exit 2`, or it provides no enforcement.
...
-   **Session events:** `SessionStart` and `Stop` fire when a session begins or ends. Use these for setup, teardown, and notifications.
-   **Tool events:** `PreToolUse` and `PostToolUse` fire before and after Claude uses a tool (writing a file, running a bash command, or searching code). These are the most powerful hooks because they can inspect and block specific actions.
-   **Notification events:** `Notification` fires when Claude generates a notification. Useful for routing alerts to Slack, desktop notifications, or logging systems.
-   **Subagent events:** `SubagentStop` fires when a subagent (spawned via the Agent tool) completes.[4](#fn:4) Hooks fire for subagent actions too, so your safety gates apply recursively.
...
-   **Project-level:** `.claude/settings.json` in your repository root (shared with your team)[3](#fn:3)
-   **User-level:** `~/.claude/settings.json` (your personal hooks, applied globally)[3](#fn:3)

The JSON structure:

`{   "hooks": {     "PreToolUse": [       {         "matcher": "Bash",         "hooks": [           {             "type": "command",             "command": "/path/to/your/script.sh"           }         ]       }     ],     "PostToolUse": [       {         "matcher": "Write|Edit",         "hooks": [           {             "type": "command",             "command": "/path/to/another-script.sh"           }         ]       }     ]   } }`
...
`{   "hooks": {     "PostToolUse": [       {         "matcher": "Write|Edit",         "hooks": [           {             "type": "command",             "command": "bash -c 'if [[ \"$FILE_PATH\" == *.py ]]; then black --quiet \"$FILE_PATH\" 2>/dev/null; elif [[ \"$FILE_PATH\" == *.js ]] || [[ \"$FILE_PATH\" == *.ts ]]; then npx prettier --write \"$FILE_PATH\" 2>/dev/null; fi'"           }         ]       }     ]   } }`
...
`{   "hooks": {     "PreToolUse": [       {         "matcher": "Bash",         "hooks": [           {             "type": "command",             "command": "bash -c 'INPUT=$(cat); CMD=$(echo \"$INPUT\" | jq -r \".tool_input.command\"); if echo \"$CMD\" | grep -qE \"rm\\s+-rf\\s+/|git\\s+push\\s+(-f|--force)\\s+(origin\\s+)?main|git\\s+reset\\s+--hard|DROP\\s+TABLE|:(){ :|:& };:\"; then echo \"BLOCKED: Dangerous command detected: $CMD\" >&2; exit 2; fi'"           }         ]       }     ]   } }`
...
Blocked patterns: - `rm -rf /` (recursive deletion from root) - `git push --force main` and `git push -f main` (force pushing to the main branch) - `git reset --hard` (destroying uncommitted work) - `DROP TABLE` (accidental database destruction) - Fork bombs
...
`{   "hooks": {     "PostToolUse": [       {         "matcher": "Write|Edit",         "hooks": [           {             "type": "command",             "command": "bash -c 'if [[ \"$FILE_PATH\" == *.py ]] && [[ \"$FILE_PATH\" != *test_* ]]; then TEST_FILE=\"tests/test_$(basename \"$FILE_PATH\")\"; if [[ -f \"$TEST_FILE\" ]]; then python -m pytest \"$TEST_FILE\" -x --tb=short 2>&1 | tail -20; fi; fi'"           }         ]       }     ]   } }`
...
`{   "hooks": {     "Stop": [       {         "matcher": "",         "hooks": [           {             "type": "command",             "command": "osascript -e 'display notification \"Claude Code session ended\" with title \"Claude Code\"'"           }         ]       }     ]   } }`
...
`curl -s -X POST "$SLACK_WEBHOOK_URL" \   -H 'Content-type: application/json' \   -d '{"text": "Claude Code session ended"}'`
...
`{   "hooks": {     "PreToolUse": [       {         "matcher": "Bash",         "hooks": [           {             "type": "command",             "command": "bash -c 'INPUT=$(cat); CMD=$(echo \"$INPUT\" | jq -r \".tool_input.command\"); if echo \"$CMD\" | grep -qE \"^git\\s+commit\"; then if ! LINT_OUTPUT=$(ruff check . --select E,F,W 2>&1); then echo \"LINT FAILED -- fix before committing:\" >&2; echo \"$LINT_OUTPUT\" >&2; exit 2; fi; fi'"           }         ]       }     ]   } }`
...
1.  **Test scripts independently first.** Pipe sample JSON into your script manually: `echo '{"tool_input":{"command":"git commit -m test"}}' | bash your-hook.sh`. If it fails outside Claude Code, it fails inside it.
2.  **Use stderr for debug output.** Anything your hook writes to stderr appears in Claude’s context. Write `echo "DEBUG: matched $CMD" >&2` while developing, then remove it once the hook is solid.
3.  **Watch for jq failures.** If your JSON path is wrong, `jq`[9](#fn:9) returns `null` silently and your conditionals will not match. Test your `jq` expressions against real tool input.
4.  **Verify exit codes.** Exit 2 blocks actions. Exit 1 only warns. A PreToolUse hook that accidentally uses `exit 1` provides zero enforcement while appearing to work. Start permissive (exit 0 by default) and use `exit 2` only for specific blocked patterns.
5.  **Keep hooks fast.** Hooks run synchronously. A hook that takes 5 seconds adds 5 seconds to every matched tool use. I keep all my hooks under 2 seconds, ideally under 500 milliseconds.
...
1.  Anthropic, “Claude Code Hooks: Lifecycle Events.” [docs.anthropic.com/en/docs/claude-code/hooks](https://docs.anthropic.com/en/docs/claude-code/hooks) [↩](#fnref:1 "Jump back to footnote 1 in the text")[↩](#fnref2:1 "Jump back to footnote 1 in the text")[↩](#fnref3:1 "Jump back to footnote 1 in the text")
    
2.  Anthropic, “Claude Code Hooks: Exit Codes.” [docs.anthropic.com/en/docs/claude-code/hooks#exit-codes](https://docs.anthropic.com/en/docs/claude-code/hooks#exit-codes) [↩](#fnref:2 "Jump back to footnote 2 in the text")[↩](#fnref2:2 "Jump back to footnote 2 in the text")[↩](#fnref3:2 "Jump back to footnote 2 in the text")[↩](#fnref4:2 "Jump back to footnote 2 in the text")
    
3.  Anthropic, “Claude Code Hooks: Configuration.” [docs.anthropic.com/en/docs/claude-code/hooks#configuration](https://docs.anthropic.com/en/docs/claude-code/hooks#configuration) [↩](#fnref:3 "Jump back to footnote 3 in the text")[↩](#fnref2:3 "Jump back to footnote 3 in the text")[↩](#fnref3:3 "Jump back to footnote 3 in the text")
    
4.  Anthropic, “Claude Code Hooks: Subagent Events.” [docs.anthropic.com/en/docs/claude-code/hooks#subagent-hooks](https://docs.anthropic.com/en/docs/claude-code/hooks#subagent-hooks) [↩](#fnref:4 "Jump back to footnote 4 in the text")[↩](#fnref2:4 "Jump back to footnote 4 in the text")[↩](#fnref3:4 "Jump back to footnote 4 in the text")
    
5.  Anthropic, “Claude Code Overview.” [docs.anthropic.com/en/docs/claude-code/overview](https://docs.anthropic.com/en/docs/claude-code/overview) [↩](#fnref:5 "Jump back to footnote 5 in the text")
    
6.  Anthropic, “Claude Code Hooks: Hook Input.” [docs.anthropic.com/en/docs/claude-code/hooks#hook-input](https://docs.anthropic.com/en/docs/claude-code/hooks#hook-input) [↩](#fnref:6 "Jump back to footnote 6 in the text")
    
7.  Anthropic, “Claude Code: Source and Hook Schema.” [github.com/anthropics/claude-code](https://github.com/anthropics/claude-code) [↩](#fnref:7 "Jump back to footnote 7 in the text")
    
8.  Git Documentation, “Customizing Git: Git Hooks.” [git-scm.com/book/en/v2/Customizing-Git-Git-Hooks](https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks) [↩](#fnref:8 "Jump back to footnote 8 in the text")
    
9.  jq Manual, “Command-line JSON processor.” [jqlang.github.io/jq/manual](https://jqlang.github.io/jq/manual/) [↩](#fnref:9 "Jump back to footnote 9 in the text")
...
29 min read](/blog/codex-vs-claude-code-2026)[
...
10 min read](/blog/claude-code-hooks)